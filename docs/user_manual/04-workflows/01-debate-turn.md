...
| **긴급 종료** | 발언 도중이라도 즉시 끊고, **이번 턴이 남긴 것을 기록에서 지움**. 보낸 글은 입력창으로 돌아옴 |
| **취소** (`cancel`) | 태스크를 죽임. 합성이 돌지 않아 산출물이 나오지 않음 (화면에는 없는 내부 동작) |

정지가 취소보다 나은 이유입니다.

### 정지와 긴급 종료

둘 다 토론을 멈추지만 **왜 멈추는가**가 다릅니다.

| | 정지 | 긴급 종료 |
| :--- | :--- | :--- |
| 전제 | 논의는 옳았다, 여기까지로 충분하다 | **요청 자체가 틀렸다** |
| 진행 중인 발언 | 끝까지 받음 | 즉시 끊음 |
| 최종 합성 | 실행 | 하지 않음 |
| 기록 | 남김 | 이번 턴이 만든 발언·도구 기록·산출물을 **지움** |
| 보낸 글 | 그대로 | **입력창으로 돌아옴** (고쳐서 다시 보냄) |

긴급 종료로 첫 요청을 지워 대화가 비면, 그 대화는 **시작 전 상태로 되돌아갑니다** —
페르소나 잠금이 풀려 에이전트 구성을 다시 만질 수 있습니다. 굳혀 둔 구성
스냅샷(`session_agents`)은 남고, 다음 턴이 시작될 때 그 시점의 `conf.json` 으로 다시
굳습니다.

> 되돌릴 수 없는 삭제라 확인 창을 한 번 거칩니다. 지우는 범위는 **그 턴이 만든
> 것**뿐이고, 앞선 턴의 대화는 건드리지 않습니다.
