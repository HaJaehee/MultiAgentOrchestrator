...
카드를 하나라도 **펼쳐 두면** 자동 스크롤이 멈춥니다. 펼친 이유는 그것을 읽기
위해서인데 다른 에이전트의 글자는 계속 도착하고, 청크마다 맨 아래로 끌려가면
읽던 자리를 잃습니다. 전부 접혀 있으면 볼 것이 흐름뿐이므로 따라갑니다.

멈춰 있는 동안에는 상태 막대에 `맨 아래로 (N개 펼침)` 이 떠서 왜 멈췄는지와
돌아가는 방법을 함께 알립니다. 화면이 사람의 카드를 대신 접지는 않습니다.
