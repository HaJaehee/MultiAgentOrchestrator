...
>
> 파일: `app/orchestration/strategies.py` (281줄)

전략은 두 가지를 정합니다: **누가 어떤 순서로 발언하는가**, 그리고 **각 발언
차례에 어떤 지침이 붙는가.** 두 번째가 전략들의 실제 차이입니다.

전략 하나(`parallel_dispatch`)는 세 번째도 정합니다: **한 명씩 말하는가, 다 같이
말하는가.**
...
### 병렬 지시 (`parallel_dispatch`)

오케스트레이터가 **겹치지 않는 과업**을 나눠 주고, 지목된 에이전트들이 **동시에**
각자의 일을 합니다. 라운드 끝에 오케스트레이터가 결과를 취합합니다.

```text
라운드 1:
              ┌──▶ 아키텍트  "인증 흐름을 설계한다"      ─┐
오케스트레이터 ├──▶ 엔지니어  "토큰 갱신 스켈레톤 작성"   ─┼─▶ 오케스트레이터
    (지시)     └──▶ 리뷰어    "세션 고정 공격을 검토한다"  ─┘        (취합)
                        같은 시각에 실행 · 서로의 답은 못 봄
```

다른 세 전략은 한 명이 끝나야 다음 사람이 시작하므로 라운드 시간은 **발언 시간의
합**입니다. 여기서는 **가장 느린 한 명의 시간**입니다.

```python
class ParallelDispatchStrategy(BaseDebateStrategy):
    orchestrator_dispatches_parallel = True   # 엔진이 라운드째로 다르게 돎
```
...
| 받는 것 | 내용 |
| :--- | :--- |
| 내 과업 | "[병렬 지시] 오케스트레이터가 이번 라운드에 당신에게 맡긴 과업입니다: ..." |
| 동시 진행 현황 | 누가 무엇을 맡았는지의 **목록** (결과는 아님) |
| 주의 | "남의 과업을 대신 하지 말고, 다른 과업의 결과가 필요하면 추측해 채우지 말고 어떤 가정을 두었는지 명시하세요" |

같은 라운드의 **답**은 아무도 못 봅니다. 이 라운드의 프롬프트는 전부 **첫 코루틴이
뜨기 전에** 만들어집니다 — 코루틴 안에서 만들면 먼저 끝난 동료의 발언이 늦게
시작한 쪽의 맥락에 섞여, 같은 라운드인데 누구는 남의 답을 보고 누구는 못 보게
됩니다. 그건 병렬이 아닙니다.

**취합이 반드시 붙는 이유**

아무도 서로를 못 봤으므로, 라운드 끝의 오케스트레이터 발언이 그 라운드를 통째로
본 유일한 자리입니다. 이 발언은 ① 통합된 현재 결론 ② 어긋나는 지점과 그 판정
③ 검증되지 않은 가정 ④ 다음 라운드로 넘길 과제를 적고, **다음 라운드 지시의
입력**이 됩니다. 이것이 없으면 라운드는 독백 묶음이 되고 모순은 최종 합성에서야
처음 발견됩니다.

**동시 실행 상한**

로스터 패널의 **동시 실행** 숫자(`sessions.parallel_limit`, 기본 3)가 한 번에
띄우는 수를 제한합니다. 상한을 넘는 지시는 **버려지지 않고 순차적으로 밀립니다.**
로컬 단일 엔드포인트(Ollama·vLLM·LM Studio)에 동시 요청을 다섯 개 던지면 큐에
쌓이거나 메모리가 터지고, 그 실패는 "에이전트가 응답하지 못했다" 로 나타납니다.

**물러섬**

| 상황 | 동작 |
| :--- | :--- |
| 엔드포인트가 죽었거나 응답에서 아는 키를 못 찾음 | 과업 없이 **전원 동시 실행**, 물러섰다는 사실을 피드에 기록 |
| `assignments` 형식이 깨짐 | 본문에서 아는 키를 긁어 과업 없이 진행 |
| 활성 전문가가 한 명뿐 | 분배 호출 자체를 하지 않음 (나눌 것이 없음) |

**개입과 정지는 라운드 경계에서만** 반영됩니다. 다른 전략은 발언과 발언 사이에서
볼 수 있지만, 여기서는 그 '사이' 에 전원이 이미 달리고 있습니다. 라운드 도중의
정지 요청은 취합을 건너뛰고 곧장 최종 합성으로 갑니다.
...
| 서로 기다릴 필요 없는 작업이 여러 갈래 (설계·구현·보안 검토를 동시에) | **병렬 지시** |
| 라운드 시간을 줄이고 싶음 (한 명씩 기다리지 않음) | **병렬 지시** |
...
- LLM 을 불러야 하면 전략에서 부르지 말고 플래그를 켜세요. 발언자를 물어볼 때는
  `orchestrator_selects_speakers`, 라운드를 동시에 돌릴 때는
  `orchestrator_dispatches_parallel` — 호출과 물러섬은 엔진이 맡습니다
