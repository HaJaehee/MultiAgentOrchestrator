...
6. **멀티 에이전트 토론 & 상태 머신 오케스트레이션**:
   ...
   - **4가지 토론 전략**: 자유 토론 (`free_debate`), 순차 검증 (`sequential_review`), 디베이트 (`adversarial_debate`), 오케스트레이터 지명 (`orchestrator_led`).
...
│   │   ├── strategies.py     # 자유 토론, 순차 검증, 디베이트, 오케스트레이터 지명 전략
...
| 전략 | 순서 | 발언 지침 |
|------|------|-----------|
| 자유 토론 | `debate_priority` 순, 전원 | 없음 — 각자 자기 관점에서 |
| 순차 검증 | `debate_priority` 순, 전원 | **직전 발언자를 이름으로 지목**해 그 결론을 이어받아 검증하도록 요구 |
| 디베이트 | `proponent` ↔ `critic` 번갈아, 각 진영 안에서는 우선순위 순, `neutral` 은 뒤 | 진영에 맞는 지침 (제안·방어 / 반례·근거) |
| 오케스트레이터 지명 | 매 라운드 오케스트레이터가 결정 | 지목 사유를 먼저 처리하도록 요구 |

> 자유 토론과 순차 검증은 **순서가 같습니다.** 두 전략의 실제 차이는 발언 지침입니다.
> 순서만 다르게 하던 예전 구조에서는 이 둘이 사실상 같은 전략이었습니다.
...
### 에이전트 풀 편집 (추가 · 비활성화 · 삭제)

로스터 패널의 **에이전트 추가** 버튼과 각 카드의 ⋮ 메뉴로 `conf.toml` 의 `[agents.*]` 를
화면에서 직접 고칠 수 있습니다. 앱을 다시 띄울 필요는 없습니다.

페르소나 편집과는 **적용 범위가 다릅니다.** 페르소나(이름·역할·시스템 프롬프트)는 대화별
오버라이드지만, 에이전트를 만들고 없애는 것은 배포 설정이라 `conf.toml` 이 정본이고 다음
기동에서도 그대로 살아 있습니다.

| 상태 | 편집 |
|------|------|
| 이 대화가 아직 시작되지 않았고 아무 대화도 토론 중이 아님 | 🟢 추가 · 비활성화 · 삭제 가능 |
| 이 대화에 이미 첫 메시지를 보냄 | 🔒 잠김 — 참여 에이전트가 고정되었습니다 |
